...
namespace Jint.Native {
    ...
    public class ClrImplDefinition<T> : JsFunction
        ...
        public ClrImplDefinition(Delegates.Func<T, JsInstance[], JsInstance> impl, JsObject prototype)
            : this(impl, -1, prototype) {
        }

        public ClrImplDefinition(Delegates.Func<T, JsInstance[], JsInstance> impl, int length, JsObject prototype)
            : this(true, prototype) {
            this.impl = impl;
            this.length = length;
        }

        public ClrImplDefinition(Delegates.Func<T, JsInstance> impl, JsObject prototype)
            : this(impl, -1, prototype) {
        }

        public ClrImplDefinition(Delegates.Func<T, JsInstance> impl, int length, JsObject prototype)
